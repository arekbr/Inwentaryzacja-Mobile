...
🔜 **Plan:**
- Deploy backendu na hosting publiczny (do tej pory tylko macos-dev Studio + linux-dev Linux dev)
...
- **Multi-device sync:** linux-dev + macos-dev + serwer. PRZED pracą `git fetch origin --prune` + sanity check `git ls-remote` vs `git log origin/X`.
